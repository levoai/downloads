...
assert_contains "GAR key: wheel force-reinstalled"       "$A" "PIP|--force-reinstall"
assert_eq       "GAR key: two install passes of the wheel (force, then deps)" "$(printf '%s\n' "$A" | grep -c '^PIP|.*shadownet-.*\.whl$')" "2"
